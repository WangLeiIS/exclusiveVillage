...
主理人 Agent 是一个特殊的系统级 Agent，具有全局视野，负责系统导航、任务协调和问题诊断。其数据存储在预置的数据库文件中，而配置（显示名和系统提示词）通过代码和数据库混合管理。

## 核心功能

- **主理人初始化**：从预置数据库加载主理人 Agent 的数据，与代码配置合并
- **配置管理**：使用硬编码配置（PRIMARY_AGENT_CONFIG）存储显示名和系统提示词
- **数据库管理**：连接到主理人专用数据库（`__primary__` 团队）
- **信息查询**：获取主理人 Agent 的详细信息或缓存的配置
- **信息更新**：更新主理人 Agent 的角色、目标描述或金币数量
- **团队名称管理**：提供主理人团队名称用于数据库操作
- **配置缓存**：初始化时合并数据库数据和代码配置，提供快速访问
...
- `getConfig()`: 获取主理人 Agent 的配置（同步方法）
  - 返回初始化时缓存的配置数据
  - 如果未初始化，返回基础配置（包含硬编码的 displayName 和 systemPrompt）
  - 合并了数据库数据和代码配置
...
### 混合配置模式
当前实现采用**代码配置 + 数据库数据**的混合模式：

**代码配置部分**（PRIMARY_AGENT_CONFIG）：
- `displayName`: 'Ren.' - 显示名称
- `systemPrompt`: 完整的系统提示词，定义主理人的职责和工作原则

**数据库数据部分**（预置数据库）：
- 基本信息：name, role, class, status
- 业务数据：coins（金币数量）, goal_description（目标描述）
- 类型标志：is_vocal, is_user

### 数据流转
1. **初始化时**：
   - 从预置数据库加载主理人 Agent 的数据
   - 合并代码中的配置（displayName 和 systemPrompt）
   - 缓存合并后的完整配置

2. **运行时**：
   - `getConfig()` 返回缓存的合并配置（同步，快速）
   - `getPrimaryAgent()` 直接从数据库查询最新数据（异步）

3. **更新时**：
   - 只能更新数据库中的字段（role, goal_description, coins）
   - 代码配置（displayName, systemPrompt）需要修改源代码
...
- **最后更新**: 2025-04-29
- **当前版本**: 2.1
- **配置模式**: 混合配置模式（代码配置 + 数据库数据）

### 版本历史

**v2.1 (2025-04-29)** - 文档修正
- 修正文档与代码不一致的问题
- 明确 `PRIMARY_AGENT_CONFIG` 常量仍然存在
- 明确 `getConfig()` 方法仍然存在
- 更新架构说明为"混合配置模式"

**v2.0 (2025-04-29)** - 架构调整
- 移除 `ensureDatabase()` 方法
- 移除 `createPrimaryAgent()` 方法
- DatabaseManager 自动处理预置数据库复制
- 新增配置缓存机制
